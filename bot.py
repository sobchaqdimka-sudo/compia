...
def _detect_user_language(history):
    """Угадать язык собеседника ('ru' или 'uk'). Приоритет — последнему сообщению.

    Раньше эвристика смотрела на все последние 6 сообщений вместе. Если юзер
    долго писал по-украински, потом перешёл на русский — детектор «помнил»
    украинский и не давал переключиться. Теперь:

    - Если в последнем содержательном (>=4 симв) user-сообщении нет украинских
      букв — отвечаем по-русски. Если есть (ї/є/і/ґ) — украинский. Это даёт
      мгновенный switch.
    - Если последнее слишком короткое («да», «ага», «ок») — смотрим на 3
      последних user-сообщения для контекста.
    ...
    if len(last) >= 4:
        return "uk" if any(c in last for c in "їєіґ") else "ru"

    # Слишком короткое — берём больше контекста.
    recent = " ".join(user_msgs[-3:]).lower()
    return "uk" if any(c in recent for c in "їєіґ") else "ru"
