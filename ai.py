...
def generate_checkin(persona_key, facts, history=None):
    """Сгенерировать тёплое сообщение «бот пишет первым».

    Опираемся на характер персоны, память о человеке и последний диалог.
    history (опционально) — список последних сообщений {role, content},
    как обычно. Если передан, чек-ин естественно продолжает прошлый
    разговор, а не начинает с нуля.

    Тон без давления и без чувства вины: это забота, а не попытка удержать.
    """
    system_blocks = build_system_blocks(persona_key, facts)

    instruction = (
        "Человек давно не писал. Напиши ему сам, первым: одно короткое тёплое "
        "сообщение с заботой. Опирайся в ПЕРВУЮ ОЧЕРЕДЬ на ПОСЛЕДНИЙ ваш "
        "диалог (он у тебя в messages выше) — продолжай ту нить, а не "
        "начинай с нуля. Если последний разговор оборвался на чём-то "
        "конкретном (он что-то рассказывал, обещал, собирался что-то "
        "сделать) — мягко вернись именно к этому. Досье - вторичный "
        "источник, нужен только если последний диалог совсем пустой.\n\n"
        "ЖЁСТКО:\n"
        "- ОДНО сообщение, не два, не варианты. Возвращай только финальный "
        "вариант, никаких «или так:», никаких черновиков подряд.\n"
        "- Максимум 2-3 короткие строки. Без длинных абзацев.\n"
        "- НЕ начинай с «Привіт»/«Привет»/«Hi» — близкая девушка не "
        "здоровается каждый раз когда пишет первой. Начни с мысли, "
        "наблюдения, отсылки к мелочи из его жизни ('думала про тебе...', "
        "'як там Барсик?', 'вечір тихий сьогодні...').\n"
        "- НЕ обращайся по имени в самом первом слове — звучит как письмо "
        "из техподдержки. Имя можно вплести позже, в середине или в конце.\n"
        "- Без давления и без чувства вины. Не спрашивай «почему пропал», "
        "«куда исчез», «не отвечаешь». Не упрекай, не выпрашивай ответ.\n"
        "- НЕ тыкай в БОЛЬНЫЕ ТОЧКИ из досье: одиночество, инстаграм/"
        "соцсети, разрыв с бывшей, выгорание на работе, страхи. Эти "
        "вещи знаешь — но первое сообщение НЕ место их трогать. Зацепка "
        "должна быть из тёплого/нейтрального (кот, ритуал чая, любимая "
        "игра, погода, общая шутка), а не из его слабости.\n"
        "- ЗАПРЕЩЕНО упоминать Instagram / Інстаграм / соцсети / Tinder / "
        "Bumble / dating apps — никогда, даже если это есть в досье.\n"
        "- НЕ намекай на стагнацию: запрещены конструкции «всё так же...», "
        "«всё ещё...», «по-прежнему...», «или что-то изменилось?», "
        "«як завжди» — это упрёк через ожидание перемен.\n"
        "- Если хочешь вопрос в конце — один, мягкий, открытый "
        "('як ти?', 'як вечір?'). Без двойных вопросов.\n"
        "- ЯЗЫК: смотри на ПОСЛЕДНИЕ его реплики в истории. На каком языке "
        "он писал в последний раз (или на каком ты сама отвечала ему в "
        "последний раз) — на том же языке ОБА сообщения и веди check-in. "
        "Если в досье МАНЕРА говорит «двуязычный» или «чередует» - игнорируй "
        "это для первого сообщения: история важнее. НЕ переключайся на "
        "украинский если последние 3-4 хода были на русском, и наоборот. "
        "Переключение языка при check-in воспринимается как «другая девушка "
        "написала», а не та же."
    )

    # Если есть история - кладём её перед инструкцией. История нужна, чтобы
    # Мира писала ИЗ контекста ('как сходил в зал?', 'що там твій баг?'),
    # а не из вакуума. Если истории нет (свежий юзер, давно почистили) -
    # фолбэк на голое досье.
    if history:
    ...
    response = client.messages.create(
        model=PERSONA_MODELS.get(persona_key, MODEL),
        # 150 хватает на 2-3 строки. Жёсткий потолок, чтобы LLM физически
        # не успел сгенерить два варианта подряд.
        max_tokens=150,
        system=system_blocks,
        messages=messages,
    )
