...
def detect_media_request(history, current_text):
    ...
    prompt = (
        ...
        "Что человек хочет ПРЯМО СЕЙЧАС? Выбери одно:\n"
        "- photo: получить/изменить её фото (поза, одежда, в полный рост, селфи, "
        "«покажись», продолжения вроде «стань боком», «переоденься»);\n"
        "- talking: видео-кружок, где она ГОВОРИТ голосом (просит услышать её голос, "
        "«скажи голосом», «озвучь», хочет, чтобы она ответила голосом или на видео);\n"
        "- video: видео-кружок без акцента на речь («запиши відео», «кружочек»);\n"
        "- none: обычный разговор, комплимент или вопрос без просьбы о фото/видео "
        "(например «красивый голос у тебя» — это none).\n"
    ...
    system_blocks.append(
        {
            "type": "text",
            "text": (
                "Сейчас ты записываешь короткий ГОВОРЯЩИЙ видео-кружочек. Ответь так, "
                "как ответила бы в чате, оставаясь полностью согласованной со всем, что "
                "уже говорила о себе раньше (НЕ придумывай новых противоречащих "
                "деталей). Дай ОДНУ короткую живую реплику (1-2 фразы), которую "
                "скажешь вслух, на языке собеседника. БЕЗ эмодзи и без описаний "
                "действий - только сама произносимая речь."
            ),
        }
    )
    response = client.messages.create(
