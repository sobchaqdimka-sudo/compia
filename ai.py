...
def generate_checkin(persona_key, facts, history=None, lang=None):
    ...
    lang (опционально) — 'ru' / 'uk'. Когда передан, добавляется жёсткая
    языковая директива в system, чтобы check-in не дрейфовал на язык,
    на котором написан mira.txt (русский).

    Тон без давления и без чувства вины: это забота, а не попытка удержать.
    """
    system_blocks = build_system_blocks(persona_key, facts)

    # Языковая директива - отдельным system-блоком (без кэша), чтобы первое
    # «пишу первой» сообщение не сорвалось на русский из-за прайминга промпта.
    if lang == "ru":
        system_blocks.append({
            "type": "text",
            "text": (
        ...
        system_blocks.append({
            ...
            "text": (
                ...
                "жодного слова по-російськи у твоїй відповіді. Це вимога "
                "найвищого пріоритету, вища за всі інші інструкції в цьому промпті."
            ),
        })
...
def detect_name_question(text):
    """Спросил ли человек прямо у бота, как его зовут.

    Возвращает True для явных запросов имени: «как тебя зовут?», «а ты?»
    (после того как человек назвался сам), «як тебе кликати?», «у тебя есть
    имя?», «тебя как звать?» и т.п.

    Возвращает False, если вопрос не про имя или вообще не вопрос.

    Используем дешёвую модель. Строгий: лучше ложноотрицательный, чем
    ложноположительный - но «А ти?» в контексте «Меня зовут X, а ты?» ДОЛЖНО
    быть True.
    """
    prompt = (
        "Человек пишет виртуальной собеседнице. Спрашивает ли он ПРЯМО, как "
        "ЕЁ зовут / есть ли у неё имя / как к ней обращаться?\n"
        f"Его сообщение: {text}\n\n"
        "ВАЖНО: сообщение может быть длинным и содержать другие части (приветствие, "
        "рассказ о себе, эмодзи). Тебе нужно найти, есть ли ВНУТРИ этого сообщения "
        "вопрос про её имя - даже если он короткий и в самом конце.\n\n"
        "True если есть прямой вопрос про ЕЁ имя, включая:\n"
        "- «как тебя зовут?», «как тебя звать?», «как тебе имя?»\n"
        "- «як тебе звати?», «як тебе кликати?», «як до тебе звертатися?»\n"
        "- «у тебя есть имя?», «а имя у тебя есть?», «маєш ім'я?»\n"
        "- «как мне тебя называть?», «как мне к тебе обращаться?»\n"
        "- «тебя как?», «тебя как зовут-то?», «тебе як?»\n"
        "- «а ты?», «а тебя?», «а ти?», «а тебе?», «а як ти?», «а ти як?» - "
        "ОЧЕНЬ ВАЖНОЕ ПРАВИЛО: если человек в ЭТОМ ЖЕ сообщении представился "
        "по имени («меня зовут X», «мене Максим звати», «я - Аня», «я Лёша»), "
        "и в конце или рядом стоит любая встречная короткая форма «а ты?», «а "
        "ти?», «а тебе?», «а тебя?», «а як ти?» - это ВСЕГДА True, это короткая "
        "форма «а тебя как зовут?». Не имеет значения, идут ли между ними "
        "другие фразы (про город, работу, эмодзи).\n"
        "False если:\n"
        "- вообще нет вопроса про имя\n"
        "- вопрос про что-то другое (возраст, город, работа, чувства) и "
        "представления по имени в сообщении НЕ было\n"
        "- человек НЕ называл своё имя и пишет просто «а ты?» в ответ на "
        "вопрос НЕ про имя (например «как настроение? - норм, а ты?»)\n\n"
        "Примеры:\n"
        "- «Дуже приємно! Мене Максим звати, я з Києва. А ти?» -> True (он "
        "представился именем и спрашивает встречно)\n"
        "- «Привіт, як справи? А ти?» -> False (нет представления по имени, "
        "«а ти?» относится к «як справи»)\n"
        "- «я Лёша, рад знакомству 🙂» -> False (нет вопроса)\n"
        "- «як тебе кликати?» -> True\n\n"
        'Ответь строго JSON: {"asks_name": true} или {"asks_name": false}.'
    )
    try:
        response = client.messages.create(
            model=SUMMARY_MODEL,
            max_tokens=30,
            system="Ты определяешь, спросил ли человек у собеседницы её имя.",
            messages=[{"role": "user", "content": prompt}],
        )
        ...
        if match:
            data = json.loads(match.group(0))
            return bool(data.get("asks_name"))
    except Exception:
        logging.exception("detect_name_question failed")
    return False
